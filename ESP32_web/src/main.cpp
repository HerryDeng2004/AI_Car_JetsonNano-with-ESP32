...
" <div id=\"menu\">\n" +
" <b>菜单：</b><br>\n" +
" <button onclick=\"ledoff()\">开灯</button><br>\n" +
" <button onclick=\"ledon()\">关灯</button><br>\n" +
" <button onclick=\"change()\">点击改变数值</button>\n" +
" <div class=\"info\">当前环境温度：<span class=\"data-display\" id=\"TemperatureIntensity\">00</span>℃</div>\n" +
" <div class=\"info\">当前环境湿度：<span class=\"data-display\" id=\"HumidityIntensity\">00</span>%</div>\n" +
...
" var Temperature = obj.Temperature.toFixed(1); "
" document.getElementById(\"TemperatureIntensity\").innerHTML = Temperature;\n" +
...
" debugMessages.unshift('温度: ' + Temperature + ' 湿度: ' + obj.Humidity + ' 光照: ' + obj.light + \"<br>\");\n" +
...
void setup() {
    ...
    //dhtSensor.begin();//DHT11.setup(T_S_pin); //初始化温湿度传感器
...
void loop() {
   ...
   {
     time2_flag=0;
     if(((int)dht.readHumidity()<500)&&((int)dht.readTemperature()<500))//滤除错误数据
      {
      Humidity=dht.readHumidity();
      Temperature=((int)(dht.readTemperature()*10))/10.0;
